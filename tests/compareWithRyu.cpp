...
int main(int argc, char** argv) {
	...
	{
		// Assert round-trip: floatToString -> stringToFloat preserves bits for eb000000
		uint32_t u = 0xeb000000u;
		float v; std::memcpy(&v, &u, sizeof v);
		std::string s = Numbstrict::floatToString(v);
		float rt = Numbstrict::stringToFloat(s);
		uint32_t urt; std::memcpy(&urt, &rt, sizeof urt);
		if (urt != u) {
	...
	{
		// Explicit regression for reported mismatch versus Ryu
		// bits: 95ae43fe
		// ours: -7.038531e-26
		// ryu:  -7.0385313e-26
		uint32_t u = 0x95ae43feu;
		float v;
		std::memcpy(&v, &u, sizeof v);
		std::string ours = Numbstrict::floatToString(v);
		std::string oracle = ryuFloat(v);
	...
	}
	{
		...
		std::cout << "strtof: " << std::hex << u << std::endl;
			...
			for (float v : fEdge) {
				...
				if (ours != oracle) {
					...
					if (bits(ra) == bits(v) && bits(rb) == bits(v) && is_decimal_tie_equivalent(ours, oracle)) {
						...
						std::printf("float mismatch (edge)\nbits: %08x\n ours: %s\n ryu:  %s\n", bits(v), ours.c_str(), oracle.c_str());
						return 1;
					}
				}
	...
	if (testFloat) {
		...
		for (int i = 0; i < testCount; ++i) {
			...
			if (bits(round) != bits(v)) {
				...
				std::cout << "round: " << std::hex << bits(round) << std::endl;
				std::cout << "v: " << std::hex << bits(v) << std::endl;
			}
			assert(bits(round) == bits(v));
			float na = Numbstrict::stringToFloat(ours);
			float nb = Numbstrict::stringToFloat(oracle);
